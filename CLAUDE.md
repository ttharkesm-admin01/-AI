...
- **PWA:** sw.js (network-first สำหรับ navigation, SWR สำหรับ asset, ไม่แตะ docs.google.com) — แก้โค้ดแอปแล้วต้องเพิ่ม `CACHE_VERSION` ใน sw.js · ปัจจุบัน `cpf-v31` · navigation fallback ตอนออฟไลน์จะลองเติม `index.html` ให้คำขอแบบโฟลเดอร์ (เช่น `/oe/`) ก่อนตกไปหน้า landing
...
- **สถานะเขียว "เชื่อม Google Sheets" แต่เดือนใหม่ไม่เข้า → ให้กด "เชื่อม" แล้ววางลิงก์ชีตใหม่อีกครั้ง (บันทึกซ้ำ)** — วิธีนี้ผู้ใช้ยืนยันแล้วว่าแก้ได้จริง (22 ส.ค. 69) การบันทึกซ้ำจะเซ็ต config ใหม่ + `local.clear()` + บังคับ `loadData(true)` ดึงสดทันที · เดาสาเหตุเดิม: config ชี้ไฟล์/แท็บเก่าค้างไว้ หรือคำขอก่อนหน้าโดนแคชกลางของ gviz
- **gviz แคชฝั่ง Google ได้** — `fetch(cache:'no-store')` คุมได้แค่แคชเบราว์เซอร์ · URL เดิมเป๊ะทุกครั้งเลยอาจโดนเสิร์ฟสำเนาเก่าซ้ำ → `fromSheets()` ต่อ `&_=Date.now()` ท้าย URL ตอน fetch (PR #60) · `buildCsvUrl()` ยังคืนลิงก์สะอาดสำหรับโชว์ในโมดอล
...
- แก้โค้ดแล้วต้องเพิ่ม `CACHE_VERSION` ใน sw.js ทุกครั้ง (ปัจจุบัน `cpf-v31`)
...
- **PR #59:** ตอบคำถาม "ทำไมสวัสดิการเดือน 7 ไม่ขึ้น" + กันปัญหาซ้ำ —
  ...
  - ตรวจในเบราว์เซอร์จริง (Playwright): welfare demo = ม.ค.–ก.ค. 69, ตัวกรอง/pivot มีเดือน ก.ค., เลือก ก.ค. แล้ว KPI 8,457 ฿ / OT 809.5 ชม. 19 คน, ไม่มี pageerror · เทสต์ 15/15 ผ่าน · bump SW `cpf-v31`

- **PR #60:** ต่อจาก #59 — ผู้ใช้ต่อชีตสำเร็จแล้ว (สถานะเขียว) แต่ยังไม่มีเดือน 7
  - **หลักฐาน:** แดชบอร์ดโหลดมา **139 รายการ** ซึ่งเท่ากับผลรวมสวัสดิการ ม.ค.–มิ.ย. ของชีตจริงเป๊ะ (21+23+26+24+24+21) และยอดค่ารักษา 33,480 ตรงกับชีตทุกบาท → ต่อชีตติดจริงและ parser ถูกต้อง แต่ **รอบนั้นไม่ได้แถวเดือน 07 (23 แถว) มาด้วย** ทั้งที่ชีตมีอยู่แล้ว
  - **ทางที่แก้ได้จริง (ผู้ใช้ยืนยัน): กด "เชื่อม Google Sheets" แล้ววางลิงก์ใหม่อีกครั้ง** — บันทึกซ้ำแล้วเดือน 7 เข้ามาทันที (บันทึกไว้เป็นวิธีแก้อันดับแรกในหัวข้อ gotcha ข้างบน)
  - โค้ดที่แก้ไว้กันซ้ำ: `fromSheets()` ใส่ `&_=Date.now()` กันแคชกลางของ gviz (ปุ่มรีเฟรชธรรมดาจะได้ของสดเสมอ ไม่ต้องพึ่งการวางลิงก์ใหม่) · bump SW `cpf-v31`
...
- **⚠️ ไฟล์ต้องเป็น Google Sheets แท้ ไม่ใช่ .xlsx ที่อัปโหลดไว้ในไดรฟ์** — `fromSheets()` ใช้ endpoint **gviz** (`/gviz/tq?tqx=out:csv`) ซึ่งรองรับเฉพาะ **native Google Sheets** (`mimeType = application/vnd.google-apps.spreadsheet`) · ไฟล์ `.xlsx` ในไดรฟ์เปิดแก้ใน Sheets ได้ (Office editing mode) และ URL หน้าตาเหมือนกันเป๊ะ (`/spreadsheets/d/<id>/edit?gid=…`) แต่ gviz อ่านไม่ได้ → แดชบอร์ดตกไปใช้ข้อมูลตัวอย่างพร้อม toast error
  ...
  - sheetId ปัจจุบัน (Google Sheets แท้ ตั้งแต่ 15 ส.ค. 69): `1H-TxdaW60LE5WYNxEgWjl5AOmhUWeiuoRXngkyCG4uY` · ของเดิมที่เป็น .xlsx `1o3GBHFUArpP7l-tGXtykPqRsngW4HCf-` **เลิกใช้แล้ว**
