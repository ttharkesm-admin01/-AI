...
- **PWA:** sw.js (network-first สำหรับ navigation, SWR สำหรับ asset, ไม่แตะ docs.google.com) — แก้โค้ดแอปแล้วต้องเพิ่ม `CACHE_VERSION` ใน sw.js · ปัจจุบัน `cpf-v24` · navigation fallback ตอนออฟไลน์จะลองเติม `index.html` ให้คำขอแบบโฟลเดอร์ (เช่น `/oe/`) ก่อนตกไปหน้า landing
...
- แก้โค้ดแล้วต้องเพิ่ม `CACHE_VERSION` ใน sw.js ทุกครั้ง (ปัจจุบัน `cpf-v24`)
...
- **PR #53:** PPT ธีมเขียวโมโนโทน + กราฟเส้นแนวโน้มรายเดือน —
  ...
  - `App.drawLine(charts, id, labels, datasets)` ใน app.js (datasets = `[{label,data,color}]`) · section "แนวโน้มรายเดือน" (`#trend-block`/`#c-trend`) ทั้งสองหน้า — OE: ค่าใช้จ่าย vs เงินยืม · welfare: ค่ารักษา vs เบี้ยเลี้ยง&อื่นๆ (ยอดเงิน ไม่รวม OT) · ซ่อนเมื่อ < 2 เดือน + guard `App.drawLine` (SW cache gotcha) · ไม่ใส่ใน export/PPT (จอเท่านั้น)
  - bump SW `cpf-v24`
