...
- **PWA:** sw.js (network-first สำหรับ navigation, SWR สำหรับ asset, ไม่แตะ docs.google.com) — แก้โค้ดแอปแล้วต้องเพิ่ม `CACHE_VERSION` ใน sw.js · ปัจจุบัน `cpf-v19` · navigation fallback ตอนออฟไลน์จะลองเติม `index.html` ให้คำขอแบบโฟลเดอร์ (เช่น `/oe/`) ก่อนตกไปหน้า landing
...
- **กราฟใน export ถูก re-render เป็นธีมเขียว** โดยไม่กระทบกราฟบนจอ: `App.chartSquareImage(chart, size, colors)` (colors = array สี หรือชื่อ palette 'green') + `App.chartBarImage(chart, color)` — หน้า oe/welfare มี wrapper `donutImg(chart, colors)` / `barImg(chart, color)` พร้อม fallback `toBase64Image` กัน SW cache mismatch
...
- แก้โค้ดแล้วต้องเพิ่ม `CACHE_VERSION` ใน sw.js ทุกครั้ง (ปัจจุบัน `cpf-v19`)
