...
- **gviz แคชฝั่ง Google ได้** — `fetch(cache:'no-store')` คุมได้แค่แคชเบราว์เซอร์ · URL เดิมเป๊ะทุกครั้งเลยโดนเสิร์ฟสำเนาเก่าซ้ำได้ (อาการ: สถานะเขียว "เชื่อม Google Sheets" แต่แถวที่เพิ่งกรอกไม่เข้า) → `fromSheets()` ต่อ `&_=Date.now()` ท้าย URL ตอน fetch (PR #60) · `buildCsvUrl()` ยังคืนลิงก์สะอาดสำหรับโชว์ในโมดอล
...
- **PR #60:** ต่อจาก #59 — ผู้ใช้ต่อชีตสำเร็จแล้ว (สถานะเขียว) แต่ยังไม่มีเดือน 7
  - **หลักฐาน:** แดชบอร์ดโหลดมา **139 รายการ** ซึ่งเท่ากับผลรวมสวัสดิการ ม.ค.–มิ.ย. ของชีตจริงเป๊ะ (21+23+26+24+24+21) และยอดค่ารักษา 33,480 ตรงกับชีตทุกบาท → ต่อชีตติดจริงและ parser ถูกต้อง แต่ **รอบนั้น gviz ไม่ได้ส่งแถวเดือน 07 (23 แถว) มาด้วย** ทั้งที่ชีตมีอยู่แล้ว
  - แก้: `fromSheets()` ใส่ `&_=Date.now()` กันแคชกลางของ gviz · bump SW `cpf-v31`
  - ⚠️ ถ้าเจออาการนี้อีก ให้เช็คด้วยว่า sheetId ในโมดอล "เชื่อม" ตรงกับไฟล์ที่กำลังกรอกจริงหรือไม่ (เคยมีไฟล์เก่า/สำเนาค้างอยู่)
