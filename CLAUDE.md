...
## งานที่ทำเสร็จแล้ว (ประวัติ PR #1–#34)
...
- **PR #32:** `App.dashboardSetup(kind, cfg)` factory ใน app.js — ดึงโค้ด lifecycle ร่วม (loadData/setStatus/buildMonthFilter/bindCommonEvents ฯลฯ) ออกจาก oe/welfare; เพิ่ม `_lastAgg` cache ใน render() ทั้งสองหน้า
- **PR #33:** ปุ่ม "📋 ส่งออก Excel" ใน oe/ + welfare/ ดาวน์โหลด .xlsx ผ่าน SheetJS; เพิ่ม `U.exportExcel()` + `U.fileDate()` ใน utils.js
- **PR #34 (pending):** ปรับปรุง infographic export: OT Top 3 พร้อมชั่วโมงแยกรายเดือน (`buildOTSection` ใน export-img.js), เบี้ยเลี้ยง Top 3 ใน right panel, กราฟ max-height 145→200px
