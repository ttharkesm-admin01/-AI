...
- **PWA:** sw.js (network-first สำหรับ navigation, SWR สำหรับ asset, ไม่แตะ docs.google.com) — แก้โค้ดแอปแล้วต้องเพิ่ม `CACHE_VERSION` ใน sw.js · ปัจจุบัน `cpf-v15`
...
- **โดนัทใน export ใช้ `App.chartSquareImage(chart)`** — re-render โดนัทลง canvas จัตุรัส (380px) ก่อน export เพื่อให้รูปเต็มช่อง ไม่มีขอบว่างซ้าย-ขวาที่ Chart.js เว้นไว้ตอนวาดใน canvas แนวกว้าง · กราฟแท่งยังใช้ `chart.toBase64Image()` ตรง (กว้างเต็มช่องอยู่แล้ว)
...
- **เงินยืมทดรอง:** amount ≤ 0 ถูกตัดทิ้ง (ใส่ติดลบเพื่อหักคืน = หาย) · ฟีเจอร์ "คืนเงินยืม/ยอดสุทธิ" **ผู้ใช้ปฏิเสธ ไม่ทำ**
...
- **ที่ผู้ใช้เคยถามแล้วปฏิเสธ (อย่าเสนอซ้ำเว้นแต่ผู้ใช้ขอ):** ฟีเจอร์คืนเงินยืม/ยอดสุทธิ · บอทถ่ายรูป→ลงชีตอัตโนมัติ (Apps Script OCR / Discord / LINE)
  - (เดิม "เปลี่ยน emoji เป็น SVG icon" อยู่ในรายการนี้ → **ผู้ใช้ขอเองแล้ว ทำใน PR #43** สำหรับ UI chrome; insight/export ยังเป็น emoji)
