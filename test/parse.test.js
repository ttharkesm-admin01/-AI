...
test('Welfare wide: ฟอร์มหัวแถวเดียว ต้องไม่ทำพนักงานคนแรกหาย', () => {
  const rows = [
    ['ค่าใช้จ่ายสวัสดิการพนักงาน ประจำเดือน กุมภาพันธ์ 2569', '', '', '', ''],
    ['ลำดับ', 'ชื่อ - สกุล', 'ตำแหน่ง', 'ค่ารักษาพยาบาล', 'เบี้ยเลี้ยง'],  // หัวแถวเดียว ไม่มีแถวหน่วยใต้ป้าย
    [1, 'นายสมชาย ทองดี', 'พนักงานขับรถ', 2000, 800],
    [2, 'นางสาวมาลี ใจดี', 'ธุรการ', 0, 1200],
  ];
  const recs = DS.normalizeAny('welfare', rows);
  const names = [...new Set(recs.map(r => r.employee))];
  assert.deepStrictEqual(names, ['นายสมชาย ทองดี', 'นางสาวมาลี ใจดี'], 'ต้องได้ครบทั้งสองคน');
  assert.ok(recs.every(r => r.month === '2569-02'));
  assert.strictEqual(recs.filter(r => r.employee === 'นายสมชาย ทองดี' && r.wtype === 'ค่ารักษาพยาบาล')[0].amount, 2000);
});
